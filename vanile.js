...
function graphcomment(gc_params) {
  ...
  function getOffset(el) {
    ...
    var bodyOffset = parseInt(getComputedStyle(document.documentElement).marginTop) + parseInt(getComputedStyle(document.body).marginTop);
    return {
      top: _y + bodyOffset,
      left: _x
    };
  ...
  var params = {
    ...
    inapp: win.graphcomment_inapp || gc_params.inapp || isFacebookApp() || false,
  ...
  if (typeof window !== 'undefined' && doc) {
    ...
    if (params.overlay) {
      ...
      if (params.overlay_button) {
        var fetchCommentCount = function fetchCommentCount(cb) {
            ...
            if (this.readyState == 4) {
              if (this.status === 200) {
                try {
                  ...
                  console.error('[graphcomment counter] fail to decode response "' + this.responseText + '", using url "' + url + '"');
                  return;
                } // write count


                if (typeof resp.count !== 'undefined') cb(resp.count);
              } else if (this.status === 400) {
                console.error('[graphcomment counter] invalid url "' + url + '", this url is not rattached to a GraphComment page.');
              }
            }
          };
        ...
        var updateCommentCount = function updateCommentCount() {
          fetchCommentCount(function (count) {
        ...
        var buttonLabels = {
          fr: 'Lire et rÃ©agir',
          zh: 'é˜…è¯»å¹¶åšå‡ºååº”',
          cz: 'PÅ™eÄtÄ›te si a reagujte',
          en: 'Read and react',
          de: 'Lesen und Reagieren',
          he: '×œ×§×¨×•× ×•×œ×”×’×™×‘',
          it: 'Lettura e reazione',
          fa: 'Ø®ÙˆØ§Ù†Ø¯Ù† Ùˆ ÙˆØ§Ú©Ù†Ø´ Ù†Ø´Ø§Ù† Ø¯Ù‡ÛŒØ¯',
          pt: 'Leitura e reaÃ§Ã£o',
          es: 'Lectura y reacciÃ³n'
        };
        ...
        updateCommentCount();
        setInterval(updateCommentCount, 10 * 1000); // every 10s
      }
      ...
      window.isGraphCommentOverlay = function () {
        return thread.classList.contains('graphcomment-overlay');
      };
  ...
  function IframeBuilder(eWin, eDoc, amp) {
    ...
    if (typeof window !== 'undefined' && eDoc) {
      var fixedHeaderHeightUpdate = function fixedHeaderHeightUpdate() {
        ...
        if (window.isGraphCommentOverlay && window.isGraphCommentOverlay()) {
      ...
      win.addEventListener('message', function (event) {
        try {
          var data = _typeof(event.data) === 'object' ? event.data : JSON.parse(event.data);

          if (data.height && data.graphcommentSource && data.pid === pid) {
            // Iframe auto-resize
            iframe.height = data.height;
          } else if (typeof data.gcToken !== 'undefined') {
            try {
              if (data.gcToken) {
                window.localStorage.setItem('gc_token', data.gcToken);
              } else {
                window.localStorage.removeItem('gc_token');
              }
            } catch (e) {}
          } else if (data.scrollTo) {
            var offset = isMobile() ? 40 : 0;
            scrollParent(iframe, data.scrollTo - offset, data.duration);
            ...
            try {
              gcToken = window.localStorage.getItem('gc_token');
            } catch (e) {}
    ...
    this.postCreation = function () {
      ...
      win.addEventListener('message', function (event) {
        ...
        if (event.origin !== gc_domain) return;

        try {
          data = _typeof(event.data) === 'object' ? event.data : JSON.parse(event.data);
          frameElementTop = -iframeFound.getBoundingClientRect().top;
        } catch (e) {
          if (event.data === 'gc-activity') return; // ignore this event

          var data = _typeof(event.data) === 'object' ? JSON.stringify(event.data) : event.data;
          console.debug('[gc] failed to decode message: ' + data, _typeof(event.data), e);
          return;
        }
        ...
        if (data.info === 'sso-auth') {
          if (!window.gcSsoAuth || typeof window.gcSsoAuth !== 'function') {
            throw new Error('[graphcomment] you need to define gcSsoAuth() to provide a way to authenticate your user.');
          }

          window.gcSsoAuth();
        }

        if (data.info === 'sso-conflict') {
          if (!window.gcSsoConflict || typeof window.gcSsoConflict !== 'function') {
            throw new Error('[graphcomment]Â you need to define gcSsoConflict() to provide a way for your users to change conflicting username.');
          }
      ...
      win.addEventListener('scroll', debounce(function () {
        var threadPos = getOffset(thread);
        iframeWindow.postMessage(JSON.stringify({
          info: 'scroll-start',
          win: {
    ...
    this.createIframeUrl = function (front_url, iframe_params) {
      ...
      for (var key in iframe_params) {
        if (key === 'target') delete iframe_params.target;
        if (key.indexOf('overlay_') === 0) delete iframe_params[key];
      }

      if (win) {
        iframe_params.overlay = window.isGraphCommentOverlay && window.isGraphCommentOverlay();
  ...
  function scrollParent(iframe, insideIFrameTop, duration) {
    ...
    var t;
    var t0 = Date.now();
    var scroll = window.isGraphCommentOverlay && window.isGraphCommentOverlay() ? thread : win;
